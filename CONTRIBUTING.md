...
gstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `gstack-openclaw/` (your global install). But when you're developing gstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
a report to `gstack-openclaw/data/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
gstack-openclaw/bin/gstack-config set gstack_contributor true
```
...
2. **Check your logs:** `ls gstack-openclaw/data/contributor-logs/`
...
Skill symlink names depend on your prefix setting (`gstack-openclaw/data/config.yaml`).
...
Your choice is saved to `gstack-openclaw/data/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `gstack-openclaw/`:

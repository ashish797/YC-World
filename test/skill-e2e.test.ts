...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local gstack-openclaw/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // gstack-openclaw/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.openclaw/ycworld/company/contributor-logs/
...
`);
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.openclaw/ycworld/company/projects/', async () => {

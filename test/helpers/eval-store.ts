 ...
 * ~/.openclaw/ycworld/company/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
export function getProjectEvalDir(): string {
  try {
    ...
    const localSlug = spawnSync('bash', ['-c', 'gstack-openclaw/bin/gstack-slug 2>/dev/null || gstack-openclaw/bin/gstack-slug 2>/dev/null'], {

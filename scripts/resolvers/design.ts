...
**Project-scoped:**
\`\`\`bash
eval "$(gstack-openclaw/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.openclaw/ycworld/company/projects/$SLUG
\`\`\`
Write to: \`~/.openclaw/ycworld/company/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md\`
...
MUST be saved to \`~/.openclaw/ycworld/company/projects/$SLUG/designs/\`, NEVER to \`.context/\`,
...
\`\`\`bash
eval "$(gstack-openclaw/bin/gstack-slug 2>/dev/null)"
_DESIGN_DIR=~/.openclaw/ycworld/company/projects/$SLUG/designs/mockup-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"

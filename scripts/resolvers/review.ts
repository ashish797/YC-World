 ...
 *
 * Review logs are stored locally at ~/.openclaw/ycworld/company/reviews/review-log.jsonl.
...
\`\`\`bash
gstack-openclaw/bin/gstack-review-read
\`\`\`
...
\`gstack-openclaw/${first}/SKILL.md\`
...
SLUG=$(gstack-openclaw/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.openclaw/ycworld/company/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.openclaw/ycworld/company/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(gstack-openclaw/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
\`\`\`bash
gstack-openclaw/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
\`\`\`bash
gstack-openclaw/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
**Persist the result:**
\`\`\`bash
gstack-openclaw/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`

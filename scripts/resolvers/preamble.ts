...
mkdir -p ~/.openclaw/ycworld/company/sessions
touch ~/.openclaw/ycworld/company/sessions/"$PPID"
_SESSIONS=$(find ~/.openclaw/ycworld/company/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.openclaw/ycworld/company/sessions -mmin +120 -type f -delete 2>/dev/null || true
...
_PROACTIVE_PROMPTED=$([ -f ~/.openclaw/ycworld/company/.proactive-prompted ] && echo "yes" || echo "no")
...
touch ~/.openclaw/ycworld/company/.completeness-intro-seen
\`\`\`
...
Always run:
\`\`\`bash
touch ~/.openclaw/ycworld/company/.proactive-prompted
\`\`\`
...
**To file:** write \`~/.openclaw/ycworld/company/contributor-logs/{slug}.md\`:
\`\`\`
# {Title}
...
if [ "$_TEL" != "off" ] && [ -x gstack-openclaw/bin/gstack-telemetry-log ]; then
  gstack-openclaw/bin/gstack-telemetry-log \\

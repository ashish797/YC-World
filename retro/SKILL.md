...
```bash
DISCOVER_BIN=""
[ -x gstack-openclaw/bin/gstack-global-discover ] && DISCOVER_BIN=gstack-openclaw/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x gstack-openclaw/bin/gstack-global-discover ] && DISCOVER_BIN=gstack-openclaw/bin/gstack-global-discover
...
ls -t gstack-openclaw/dataretros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p gstack-openclaw/dataretros
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls gstack-openclaw/dataretros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `gstack-openclaw/dataretros/global-${today}-${next}.json`:
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `gstack-openclaw/dataretros/` (not `.contextretros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
